...
console.log(Object.keys(ggf)[i])
  ...
  */ 
  ...
  zooml = window.open(crtd["start_url"]
  ...
  telenot("<b>" + "New Meeting " + localStorage.getItem("ctpic") + " is now live on Aduruthuma" + "</b>" + "\n"+"Topic: "+ crtd["topic"] + "\nClass ID: " + cid + "\n" + "Meeting ID: " + crtd["id"] + "\n" + "Start URL: " + crtd["start_url"] + "\n" + "Timestamp: " + moment().format("YYYY-MM-DD hh:mm a"))
  /*
  st.database().ref(   "live/" + classname ).update( {
    id: crtd["id"],
    sts: "Live"
  })*/
...
telenot("<b>" + "New Merged Class Started "+"</b>\n Merged to: ClassIDs "+mergeray + "\nHost Class: " +nn[cid]["topic"] )
      ...
      var data = JSON.stringify({
          ...
          "settings": {
            ...
            "cn_meeting": false,
            "contact_email": "elkalhara@gmail.com",
            "contact_name": "Nadeeja Kalhara",
...
if (JSON.parse(xhr7895411.response)["status"]=="started"){
  Swal.fire({
    ...
    if (result.isConfirmed) {
      zooml2 = window.open(JSON.parse(xhr7895411.response)["start_url"]
      ,'height=0px,width=0px,toolbar=no,menubar=no,scrollbars=no');
                  zooml2.moveTo(0, 0);
                  zooml2.blur();
          window.focus();
      ...
      if (result.isDenied) {
        ...
        telenot("<b>Force Ended Class :" + localStorage.getItem("ctpic") + "</b> \n" + " Class ID: "+cid+ "\n" + " Meeting ID: "+JSON.parse(xhr7895411.response)["id"]+ "\n Restart URL: "+JSON.parse(xhr7895411.response)["start_url"] )
            }}
            data = JSON.stringify({
              action: "end"
            })
