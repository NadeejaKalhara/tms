...
function manen (result,xz,password){
      if (result.isConfirmed) {
            Swal.fire({
                  ...
                  html: 'Writing the DB in <b></b> milliseconds.',
                  timer: 2000,
            ...
            st.auth().getUserByEmail(xz["pn"]+"@aduruthuma.lk")
            .then((userRecord) => {
          ...
          Swal.fire(
                'Enrolled!',
                "A new payment for the stundet ID: <b>"+ "AD"+("000" + password ).slice(-4) + "</b> has been done for the month of <b>"+ $$('enlist').value+"</b>" ,
                'success'
              )
              telenot(  "✅ A new payment for the stundet ID: <b>"+ "AD"+("000" + password ).slice(-4) + "</b> has been done for the month of <b>"+ $$('enlist').value+"</b>"+"\nBy "+localStorage.nm)
          })})
            })
            .catch((error) => {
...
async function enrollstx () {
      const { value: password } = await Swal.fire({
            ...
            input: 'number',
            inputLabel: 'Student ID without AD & leading zeros',
...
if(xz!=null){
                 ...
                 title:"Verify the student",
                        html:
                         ...
                         showCancelButton:true,
                        footer: '<a href="tel:0412230269">Want more info?</a>'
                      }).then((result) => {
...
function deletes  (zx){
      ...
      Swal.fire({
            title: 'Are you sure?',
            html: "All the paymnet records of the stundet ID: <b>"+ "AD"+("000" + sid ).slice(-4) + "</b> will be deleted permanently deleted for the class id "+ cid +": "+ classname+" by this action.",
            icon: 'warning',
            showCancelButton: true,
            ...
            if (result.isConfirmed) {
                  ...
                  st.database().ref("enroll/"+uid+"/"+cid+"/").set({}).then((m)=>{
                        firebase.database().ref("pay/"+cid+"/"+sid+"/").set({}).then((m)=>{
                              Swal.fire(
                                    'Student Removed!',
                                    "Stundet ID: <b>"+ "AD"+("000" + sid ).slice(-4) + "</b> has been permanently removed from the class id <b>"+ cid +": "+ classname+"</b>" ,
                                    'success'
                                  )
                                  telenot(  "⚠️ Stundet ID: <b>"+ "AD"+("000" + sid ).slice(-4) + "</b> has been permanently removed from the class id <b>"+ cid +": "+ classname+"</b>"+"\nBy "+localStorage.nm)
                        })
                  })
            }})
}
function regain (zx){
      ...
      Swal.fire({
            title: 'Are you sure?',
            html: "A new payment for the stundet ID: <b>"+ "AD"+("000" + sid ).slice(-4) + "</b> will be done for the month of <b>"+ $$('enlist').value,
            icon: 'warning',
            showCancelButton: true,
            cancelButtonColor: '#d33',
            confirmButtonText: 'Yes, Enroll!'
...
fee:clzfeeam.toString()
                  ...
                  Swal.fire(
                        'Enrolled!',
                        "A new payment for the stundet ID: <b>"+ "AD"+("000" + sid ).slice(-4) + "</b> has been done for the month of <b>"+ $$('enlist').value+"</b>" ,
                        'success'
                      )
                      telenot(  "✅ A new payment for the stundet ID: <b>"+ "AD"+("000" + sid ).slice(-4) + "</b> has been done for the month of <b>"+ $$('enlist').value+"</b>"+"\nBy "+localStorage.nm)
              })})
...
function expires (zx){
      ...
      Swal.fire({
            title: 'Are you sure?',
            html: "The payment of the stundet ID: <b>"+ "AD"+("000" + sid ).slice(-4) + "</b> done for the month of <b>"+ $$('enlist').value +"</b> will be rejected by this action",
            icon: 'warning',
            showCancelButton: true,
                        ...
                        pay:"no"
                                          ...
                                          firebase.database().ref("pay/"+cid+"/"+sid+"/"+$$('enlist').value).update({
                                                pay:"no"
                                                                  ...
                                                                  Swal.fire(
                                                                        'Expired!',
                                                                        "The payment of the stundet ID: <b>"+ "AD"+("000" + sid ).slice(-4) + "</b> done for the month of <b>"+ $$('enlist').value +"</b> has been rejected successfully",
                                                                        'success'
                                                                      )
                      telenot(   "🔴 The payment of the stundet ID: <b>"+ "AD"+("000" + sid ).slice(-4) + "</b> done for the month of <b>"+ $$('enlist').value +"</b> has been rejected successfully"+"\nBy "+localStorage.nm)
...
totalin = 0
  ...
  if(e!=null){
        ...
        cell22.innerHTML =  "AD"+("000" + e["sid"] ).slice(-4)
