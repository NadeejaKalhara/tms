...
$$("coppy").onclick=function(){
  copyTextToClipboard(JSON.parse(xhr556481.response)["join_url"])
    ...
    var data = JSON.stringify({
      "first_name": "AD-"+studenti,
      "last_name": studentn,
      "email": studenti+"@aduruthuma.lk",
      "state": "Sri Lanka",
      "comments": "FREE CARD",
      "job_title": "Student",
      "org": "Aduruthuma LMS",
...
Toast.fire({
  icon: 'success',
  title: 'Meeting Name set successfully'
})        
...
function pushrec (rd) {
  ...
  Swal.fire({
    ...
    footer: 'NOTE : Use Aduruthuma Media Player to disable cloud recording screen-capturing.'
  }).then((result) => {
    if (result.isConfirmed) {
othen()
    }})
...
var table33 = $$("atdl")
if (oo["participants"][i]["name"].includes("AD")){
