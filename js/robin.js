function phb(thz){
    if($$(thz).checked){
    console.log(thz)
  st.auth().getUserByEmail(yz["pn"]+"@aduruthuma.lk")
  .then((userRecord) => {
...
uid:uid
    ...
    telenot(  "✅ [FREE STUDENT] A new payment for the stundet ID: <b>"+ "AD"+("000" + pwd ).slice(-4) + "</b> has been done for the month of <b>"+ thz+"</b>"+"\nBy "+localStorage.nm)
})})
  })
  .catch((error) => {
  ...
  uid:uid
      ...
      telenot(  "🔴 [FREE STUDENT] Cancelled payment for the stundet ID: <b>"+ "AD"+("000" + pwd ).slice(-4) + "</b> for the month of <b>"+ thz+"</b>"+"\nBy "+localStorage.nm)
  })})
    })
    .catch((error) => {
      console.log('Error fetching user data:', error);
...
function xnxx (result,xz,password){
    result = result
    yz = xz
    pwd = password
    Swal.fire({
